function [d] = festimate(d,ndes)
%% estimate and plot the data

%%------------------------------------------------------
%% step 3: model estimation
load(d.design(ndes).fname_SPM);
...
save(d.design(ndes).fname_SPM,'SPM_nirs')

try
    [pathn, name, ext] = fileparts(d.design(ndes).fname_SPM);
    pathn = [pathn filesep];
catch
    keyboard
    idx = find(d.design(ndes).fname_SPM == filesep);
    pathn = d.design(ndes).fname_SPM(1:idx(end));
end
